...
## Status 2026-09-14 15:54:50 CEST

### Current scroll-test source

The ROM selected by `ver/game/rom.bin` is currently:

```text
ver/pcb_test/tscr4_6h_sim.rom
```

Its 6809 test program and ROM image are generated by
`ver/pcb_test/make_pcb_testrom_6h.py`. The normal entry point is:

```bash
cd cores/mzone/ver/pcb_test
./build_scroll_tests.sh
```

`build_scroll_tests.sh` generates the non-flipped `tscr4`, flipped `tscr4f`,
and slower flipped `tscr4fs` variants. `tools/make_scrolltest_rom.py` is a
separate older diagnostic generator and is not the source of the currently
selected `tscr4` ROM.

### Active-width correction

The default output is now 288 active pixels. OSD option `OD` selects between:

```text
Video width: 288
Video width: Original 287
```

The original 287-pixel option applies only in non-flipped mode. SCROLL was
already producing a valid pixel at horizontal position 287. The black final
column was caused by FIX starting its hidden end-of-line fetch/source interval
at the old 287-pixel boundary, so the color mixer selected a black FIX pixel
instead of the valid SCROLL pixel. `jtmzone_fix.v` now uses 288 for that
non-flipped boundary in default mode and retains 287 when the original-width
option is selected. Flipped geometry remains unchanged.

After the correction, the 224 pixels sampled from the rendered column at
horizontal position 287 contained 104 blue, 104 yellow, and 16 magenta pixels,
with no black pixels.

### Main and sound cleanup

`jtmzone_main.v` now follows the early-Konami/JTCORE style more closely:

- active-high decoded signals consistently use the `_cs` suffix;
- chip selects are produced in one combinational address decoder;
- CPU read data is selected in a registered priority mux;
- RAM writes are derived from chip select and `RnW` instead of separate,
  redundant write strobes;
- scroll registers are written on `cpu_cen` without the former `h2` pending
  write workaround;
- LS259 outputs use functional names such as `coin1`, `coin2`, `flip`,
  `intsnd`, and `intst`;
- IRQ and FIRQ latches use the active-low CPU inputs through the `q` outputs
  of `jtframe_ff`.

Sound is split into `jtmzone_snd.v` for the board-level address decoder and
external interfaces, and `jtmzone_snd_dev.v` for the Z80, waits, AY devices,
MCU, and audio path. The former `jtmzone_8039.v` wrapper was renamed to
`jtmzone_mcu.v`; it contains the MCS-48 core, internal data RAM, command/IRQ
handshake, timer/status handling, P1 DAC output, and external MCU ROM
interface.

### Latest verification

The latest sound-enabled seven-frame simulation completed successfully using
`tscr4_6h_sim.rom`. The active-width checker reported 288 pixels. Waveform:

```text
cores/mzone/ver/game/test.fst
timestamp: 2026-09-14 15:07:21 CEST
size:      12,629,124 bytes
```

`git diff --check` passes. The working tree also contains the separately
edited `sch/mcpu.kicad_sch` and untracked `modules/jt680x/crasm/`; preserve
those changes.
