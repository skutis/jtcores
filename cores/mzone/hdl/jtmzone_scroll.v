...
localparam [8:0] SCROLL_START_DLY = 9'd8;
localparam [8:0] FIX_HPOS_OFS = 9'd5;
localparam [8:0] SCR_HPOS_OFS = 9'd6;
...
wire [ 8:0] fix_hpos = hdump + FIX_HPOS_OFS;
wire [ 7:0] fix_hdf = flip ? ~fix_hpos[7:0] : fix_hpos[7:0];
wire [ 7:0] fix_vdf = flip ? ~vdump[7:0] : vdump[7:0];
wire [ 7:0] fix_rd_x = fix_hdf - 8'd8;
wire        fix_fetch = fix_hdf[2:0] == FETCH_PHASE;
wire        fix_load = fix_hdf[2:0] == LOAD_PHASE;
wire [11:0] fix_tile_addr = { cram1[7], vram1, fix_vdf[2:0] ^ {3{cram1[5]}} };
wire [31:0] fix_decoded_row;

wire [ 8:0] scr_hdf_base = hdump - 9'd32 + SCR_HPOS_OFS;
wire [ 8:0] scr_x_sum = {1'b0,scr_hdf_base[7:0]} + {1'b0,~scrolly};
wire [ 8:0] scr_y_sum = {1'b0,vdump[7:0]} + {1'b0,~scrollx} + 9'd1;
wire [ 7:0] scr_hdf = flip ? ~scr_x_sum[7:0] : scr_x_sum[7:0];
wire [ 7:0] scr_vdf = flip ? ~scr_y_sum[7:0] : scr_y_sum[7:0];
wire [ 7:0] scr_rd_x = scr_hdf - 8'd7;
wire        scr_fetch = !DISABLE_SCROLL && scr_hdf[2:0] == FETCH_PHASE;
...
wire [ 3:0] lut_pxl;
wire [ 3:0] lut_pen = { pxl_raw[0], pxl_raw[1], pxl_raw[2], pxl_raw[3] };
...
assign pxl = lut_pxl;
assign fix_en = display_fix;
assign dbg_hcnt = display_fix ? fix_hpos : scr_hdf_base;
assign dbg_fix = display_fix;
assign dbg_rom_addr = display_fix ? fix_tile_addr : scr_tile_addr;
assign dbg_scr_x = scr_x_sum[7:0];
assign dbg_pat_x = display_fix ? fix_hdf : scr_hdf;
assign dbg_scr_y = scr_y_sum[7:0];
assign dbg_pat_y = display_fix ? fix_vdf : scr_vdf;
